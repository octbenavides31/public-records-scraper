...
This is a sanitized version of a system I run in production across 100+ Texas counties. The real code touches actual people's records and isn't public, so this demonstrates the same engineering on invented data.
...
**Per-source adapters, not a branching parser.** Each county is a class implementing `fetch()` and `parse()`. Adding a county means adding one class; it doesn't touch — or risk — the ninety-nine already working. The alternative, one function with a growing `if county == ...` chain, means every county shares a blast radius.
...
**A polite rate limiter.** Minimum interval between requests, injectable clock and sleep so the tests verify the timing logic without actually sleeping.
...
Adapter pattern · schema normalization across heterogeneous sources · retry with jitter · rate limiting · failure isolation in batch jobs · non-destructive update strategy · dependency injection for testable time · defensive parsing.
